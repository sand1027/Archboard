 ...
 * An HLD starter.
 *
 * LLD starters live in ./lld.ts and are a different shape on purpose. This file's payload is
 * an HLD `Diagram` — `nodes` and `edges` — which the LLD canvas cannot render: it reads
 * `shapes` from lldStore. Four LLD templates used to sit here in this shape and silently did
 * nothing when clicked, so `mode` is gone and the type no longer admits them.
 */
...
export function getTemplatesByMode(mode: 'hld'): Template[] {
